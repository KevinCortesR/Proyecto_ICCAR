...
vector<vector<double>> Gauss_Seidel(double BC_N, double BC_S, double BC_W, double BC_E, vector<double> &xcr, vector<double> &ycr, vector<vector<double>> &Mat_A, vector<double> &Vec_Q, vector<double> &T, double epsilon){
	int it = 0;
	...
	while (Res_norm >= epsilon){
		...
		for (int j = 0; j < nycr; j++){
			for (int i = 0; i < nxcr; i++){
				...
				int S = nc - nxcr;
				if ((i > 0 && i < (nxcr - 1)) && j == (nycr - 1)){ // Celdas N (Tipo 1)
					T[nc] = (Vec_Q[nc] - (Mat_A[nc][1] * T[E] + Mat_A[nc][2] * BC_N + Mat_A[nc][3] * T[W] + Mat_A[nc][4] * T[S])) / Mat_A[nc][0];
				}
				else if ((i > 0 && i < (nxcr - 1)) && j == 0){ // Celdas S (Tipo 2)
					T[nc] = (Vec_Q[nc] - (Mat_A[nc][1] * T[E] + Mat_A[nc][2] * T[N] + Mat_A[nc][3] * T[W] + Mat_A[nc][4] * BC_S)) / Mat_A[nc][0];
				}
				else if (i == 0 && (j > 0 && j < (nycr - 1))){ // Celdas W (Tipo 3)
		...
		for (int j = 0; j < nycr; j++){
			for (int i = 0; i < nxcr; i++){
				...
				int S = nc - nxcr;
				if ((i > 0 && i < (nxcr - 1)) && j == (nycr - 1)){ // Celdas N (Tipo 1)
					R = R + abs(Vec_Q[nc] - (Mat_A[nc][1] * T[E] + Mat_A[nc][2] * BC_N + Mat_A[nc][3] * T[W] + Mat_A[nc][4] * T[S] + Mat_A[nc][0] * T[nc]));
				}
				else if ((i > 0 && i < (nxcr - 1)) && j == 0){ // Celdas S (Tipo 2)
					R = R + abs(Vec_Q[nc] - (Mat_A[nc][1] * T[E] + Mat_A[nc][2] * T[N] + Mat_A[nc][3] * T[W] + Mat_A[nc][4] * BC_S + Mat_A[nc][0] * T[nc]));
				}
				else if (i == 0 && (j > 0 && j < (nycr - 1))){ // Celdas W (Tipo 3)
